...
function applyGraphRoot(value) { const rootId = resolveGraphRoot(value); if (!rootId) { setGraphRoot(null); return; } const root = holons.find(holon => String(holon.id) === String(rootId)); elements.graphRoot.value = root?.name || ''; setGraphRoot(rootId); }
