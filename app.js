...
function wireUI() {
  ...
  window.addEventListener('holon:contextcreate', event => void createHolon(event.detail?.name || '', event.detail?.type || '', event.detail?.parent || null)); window.addEventListener('holon:contextedit', event => { if (event.detail?.holon) void editHolon(event.detail.holon); }); window.addEventListener('holon:contextdelete', event => { if (event.detail?.holon) void deleteHolon(event.detail.holon); }); window.addEventListener('relationship:selected', event => { if (event.detail?.id) renderRelationshipInspector(event.detail); }); window.addEventListener('relationship:contextcreate', () => void createRelationship()); window.addEventListener('relationship:contextdelete', event => { if (event.detail?.relationship) void deleteRelationship(event.detail.relationship, event.detail); });
