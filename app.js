...
function isEditableHolonProperty(key) { return !['id', 'created_at', 'created_by', 'created_by_name'].includes(key); }
...
function renderHolonInspector(holon)
{
  ...
  const decoded = decodeDynamicContent(holon.Content);
  const schemaRows = Object.entries(holon)
    .filter(([key]) => !['children', 'Content', 'created_by'].includes(key))
  ...
  const dynamicRows = dynamicFields.map(field => ({
    key: `field:${field.id}`,
    property: field.name,
    value: formatPropertyValue(decoded.fields[String(field.id)] ?? field.defaultValue),
  }));
  ...
  propertyGrid = createEBProps(elements.inspectorContent, {
    ...
    editor: row =>
    {
      ...
      if (!isEditableHolonProperty(row.key)) return null;
      return { type: 'text' };
    },
    onChange: (row, newValue) =>
    {
      ...
      {
        void saveInspectorProperty(holon, key, newValue);
        return;
      }
      const rawValue = String(newValue ?? '');
      const originalValue = propertyValueForDisplay(key, key === 'Content' ? decoded.legacyContent : holon[key]);
...
function renderRelationshipInspector(relationship) {
  ...
  if (!relationship) {
    ...
    const empty = document.createElement('div');
    empty.className = 'muted';
    empty.textContent = 'Select a Holon or relationship to inspect its properties.';
    elements.inspectorContent.appendChild(empty);
    return;
  }
...
function applyGraphRoot(value) { const rootId = resolveGraphRoot(value); if (!rootId) { storeGraphRoot(null); setGraphRoot(null); window.dispatchEvent(new CustomEvent('eB:graphRootChanged', { detail: { rootId: null } })); return; } const root = holons.find(holon => String(holon.id) === String(rootId)); elements.graphRoot.value = root?.name || ''; storeGraphRoot(rootId); setGraphRoot(rootId); window.dispatchEvent(new CustomEvent('eB:graphRootChanged', { detail: { rootId } })); }
...
function holonOptions(includeNone = false, selected = '') { const options = holons.filter(h => String(h.holon_type || '').toLowerCase() !== 'provenance').map(h => ({ value: h.id, label: h.name || '(unnamed)' })).sort((a, b) => a.label.localeCompare(b.label, undefined, { sensitivity: 'base', numeric: true })); if (includeNone) options.unshift({ value: '', label: '— None —' }); if (selected && !options.some(option => option.value === selected)) options.unshift({ value: selected, label: '(current)' }); return options; }
...
function defaultParentRelationship(typeName) { const type = String(typeName || '').trim().toLowerCase(); const preferred = type === 'circle' ? ['circle of', 'subcircle of', 'member of'] : type === 'role' ? ['role of', 'within', 'member of'] : type === 'tension' ? ['tension within', 'within', 'member of'] : type === 'process' ? ['process within', 'within', 'member of'] : ['member of', 'within']; return relationshipTypes.find(item => preferred.includes(String(item.name || '').trim().toLowerCase()))?.id || ''; }
const HIERARCHICAL_RELATIONSHIPS = new Set(['branch of', 'child of', 'circle of', 'member of', 'part of', 'process within', 'role of', 'subcircle of', 'tension within', 'within']);
function hierarchyRelationshipTypeName(relationship) { return String(relationship.relationship_type || relationship.relationship_type_name || relationshipTypes.find(type => String(type.id) === String(relationship.relationship_type_id))?.name || '').trim().toLowerCase(); }
function branchFor(rootId) { const root = String(rootId); const depth = new Map([[root, 0]]); const queue = [root]; for (let cursor = 0; cursor < queue.length; cursor += 1) { const parentId = queue[cursor]; for (const relationship of relationships) { if (String(relationship.target_holon_id) !== parentId || !HIERARCHICAL_RELATIONSHIPS.has(hierarchyRelationshipTypeName(relationship))) continue; const childId = String(relationship.source_holon_id); if (depth.has(childId)) continue; depth.set(childId, depth.get(parentId) + 1); queue.push(childId); } } const nodeIds = new Set(depth.keys()); const touchingRelationships = relationships.filter(relationship => nodeIds.has(String(relationship.source_holon_id)) || nodeIds.has(String(relationship.target_holon_id))); return { depth, descendants: queue.slice(1), touchingRelationships }; }
...
async function deleteHolon(holon) { const name = holon.name || '(unnamed)'; const branch = branchFor(holon.id); let choice; if (branch.descendants.length) { const descendantWord = branch.descendants.length === 1 ? 'descendant' : 'descendants'; const relationshipWord = branch.touchingRelationships.length === 1 ? 'relationship' : 'relationships'; choice = await eBConfirm({ title: 'Delete node or branch?', message: `“${name}” has ${branch.descendants.length} ${descendantWord}. Deleting the entire branch will remove ${branch.descendants.length + 1} nodes and ${branch.touchingRelationships.length} ${relationshipWord}.\n\nDelete Node Only leaves its descendants in place but disconnected from this node.`, choices: [{ label: 'Cancel', value: 'cancel', autofocus: true }, { label: 'Delete Node Only', value: 'node' }, { label: 'Delete Entire Branch', value: 'branch', className: 'danger' }] }); } else { choice = await eBConfirm({ title: 'Delete node?', message: `Delete “${name}”?`, confirmLabel: 'Delete' }) ? 'node' : 'cancel'; } if (choice === 'cancel') return; const ids = choice === 'branch' ? [...branch.depth.entries()].sort((left, right) => right[1] - left[1]).map(([id]) => id) : [holon.id]; setStatus(choice === 'branch' ? `Deleting ${name} and ${branch.descendants.length} descendants…` : `Deleting ${name}…`); try { for (const id of ids) await eBliss.holons.delete(id); await loadModel(); setStatus(choice === 'branch' ? `Deleted ${name} and its branch` : `Deleted ${name}`, 'success'); } catch (error) { setStatus(error.message || 'Unable to delete Holon', 'error'); } }
async function createHolonType() { const values = await showModal({ title: 'New Node Type', submitLabel: 'Create Type', fields: [{ name: 'name', label: 'Name', required: true, placeholder: 'e.g. Service' }, { name: 'description', label: 'Description', placeholder: 'What kind of node is this?' }] }); if (!values?.name?.trim()) return; const name = values.name.trim(), description = values.description?.trim() || ''; setStatus(`Creating node type ${name}…`); try { await eBliss.holonTypes.create({ name, description }); await loadModel(); setStatus(`Created node type ${name}`, 'success'); } catch (error) { setStatus(error.message || 'Unable to create node type', 'error'); } }
...
async function createHolon(prefillName = '', prefillType = '', parentHolon = null) { const type = prefillType || defaultHolonType(); if (!type) { setStatus('No node types are available', 'error'); return null; } const parentRelationship = parentHolon ? defaultParentRelationship(type) : ''; const values = await showModal({ title: 'New Node', submitLabel: 'Create Node', fields: [{ name: 'name', label: 'Name', required: true, placeholder: 'Node name', value: prefillName }, { name: 'holon_type', label: 'Type', type: 'combobox', options: holonTypeOptions(type), value: type, required: true, minChars: 0, allowCustom: false, placeholder: 'Find a node type…' }, { name: 'relationship_type_id', label: 'Initial Relationship', type: 'select', options: relationshipTypeOptions(true, parentRelationship), value: parentRelationship }, { name: 'parent_holon_id', label: 'Parent Node', type: 'select', options: holonOptions(true, parentHolon?.id || ''), value: parentHolon?.id || '' }, { name: 'position', label: 'Position', type: 'number', value: '0' }] }); if (!values?.name?.trim()) return null; if ((values.relationship_type_id && !values.parent_holon_id) || (!values.relationship_type_id && values.parent_holon_id)) { setStatus('Choose both an initial relationship and a parent node, or leave both empty', 'warn'); return null; } const name = values.name.trim(); setStatus(`Creating ${name}…`); try { let holon = await eBliss.holons.create({ name, holon_type: values.holon_type }); const pending = holon.pendingApproval === true; if (!pending) holon = await initializeDynamicFields(holon) || holon; if (values.relationship_type_id && values.parent_holon_id) await eBliss.relationships.create({ source_holon_id: holon.id, target_holon_id: values.parent_holon_id, relationship_type_id: values.relationship_type_id, position: Number(values.position) || 0 }); await loadModel(); if (pending) { setStatus(`${name} submitted for approval`, 'success'); return holon; } holon = holons.find(item => String(item.id) === String(holon.id)) || holon; openHolon(holon); setStatus(`Created ${name}`, 'success'); return holon; } catch (error) { setStatus(error.message || 'Unable to create node', 'error'); return null; } }
...
async function createRelationship(sourceHolon = null) { if (holons.length < 2 || !relationshipTypes.length) return setStatus('Need at least two Holons and one relationship type', 'warn'); const values = await showModal({ title: 'New Relationship', submitLabel: 'Create Relationship', fields: [{ name: 'source_holon_id', label: 'Source Holon', type: 'select', options: holonOptions(), value: sourceHolon?.id || '', required: true }, { name: 'relationship_type_id', label: 'Relationship', type: 'select', options: relationshipTypeOptions(), required: true }, { name: 'target_holon_id', label: 'Target Holon', type: 'select', options: holonOptions(), required: true }, { name: 'position', label: 'Position', type: 'number', value: '0' }] }); if (!values) return; setStatus('Creating relationship…'); try { await eBliss.relationships.create({ source_holon_id: values.source_holon_id, relationship_type_id: values.relationship_type_id, target_holon_id: values.target_holon_id, position: Number(values.position) || 0 }); await loadModel(); setStatus('Relationship created', 'success'); } catch (error) { setStatus(error.message || 'Unable to create relationship', 'error'); } }
...
function wireUI() {
  ...
  window.addEventListener('holon:contextcreate', event => void createHolon(event.detail?.name || '', event.detail?.type || '', event.detail?.parent || null)); window.addEventListener('holon:contextedit', event => { if (event.detail?.holon) void editHolon(event.detail.holon); }); window.addEventListener('holon:contextdelete', event => { if (event.detail?.holon) void deleteHolon(event.detail.holon); }); window.addEventListener('relationship:selected', event => { if (event.detail?.id) renderRelationshipInspector(event.detail); }); window.addEventListener('relationship:contextcreate', event => void createRelationship(event.detail?.parent || null)); window.addEventListener('relationship:contextdelete', event => { if (event.detail?.relationship) void deleteRelationship(event.detail.relationship, event.detail); });
