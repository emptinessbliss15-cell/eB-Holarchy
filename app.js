...
function relationshipTypeName(relationship) { return String(relationship.relationship_type || relationship.relationship_type_name || relationshipTypes.find(type => String(type.id) === String(relationship.relationship_type_id))?.name || '').trim().toLowerCase(); }
function branchFor(rootId) { const root = String(rootId); const depth = new Map([[root, 0]]); const queue = [root]; for (let cursor = 0; cursor < queue.length; cursor += 1) { const parentId = queue[cursor]; for (const relationship of relationships) { if (String(relationship.target_holon_id) !== parentId || !HIERARCHICAL_RELATIONSHIPS.has(relationshipTypeName(relationship))) continue; const childId = String(relationship.source_holon_id); if (depth.has(childId)) continue; depth.set(childId, depth.get(parentId) + 1); queue.push(childId); } } const nodeIds = new Set(depth.keys()); const touchingRelationships = relationships.filter(relationship => nodeIds.has(String(relationship.source_holon_id)) || nodeIds.has(String(relationship.target_holon_id))); return { depth, descendants: queue.slice(1), touchingRelationships }; }
