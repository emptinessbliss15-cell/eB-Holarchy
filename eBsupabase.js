...
export function createEBSupabase()
{
  ...
  return {
    ...
    profile: { async get(userId) { return result('Profile', await supabase.from('profiles').select('*').eq('id', userId).maybeSingle()); }, async upsert(userId, values) { return result('Profile', await supabase.from('profiles').upsert({ id: userId, ...values }).select().single()); } },
