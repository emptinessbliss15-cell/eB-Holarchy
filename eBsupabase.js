...
export function createEBSupabase()
{
  ...
  {
    const holonTypeId = await result('Holon Type type', await supabase.from('holon_types').select('id').eq('name', 'Holon Type').single());
    const provenanceHolon = result('Provenance Holon', await supabase.from('holons').select('id').eq('name', 'Provenance').eq('holon_type_id', holonTypeId.id).maybeSingle());
